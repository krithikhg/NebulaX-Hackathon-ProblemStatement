...
| Rail | v5 features (physics + localisation + shape + cyclostationary) + top-40 LightGBM, 10-seed, batch fault-budget rule | Stratified 5-fold CV (+ repeated 68-file holdouts) | 0.833 CV / **0.805** held-out |
| SHM | Rainflow + physics-informed bank Ridge | 8-fold CV, 5 seeds, 64 files | **0.994** |

Implied Overall Score (sum ÷ 4) ≈ **0.94**.
...
directory (`PS3/app/state/`). The frontend files each upload under a free-text
**component** label, set in the component field at the top of the SHM page
before uploading, and tracks fatigue separately per component.
...
(honest single-split OOF **0.833 ± 0.03**; the 0.88 figure is the optimistic
5-split-averaged OOF). Error anatomy: 5 missed, 5 false alarms, only 1 side swap,
so the difficulty is fault-vs-Normal detection rather than side localisation;
recall falls to 0.71 above 60 km/h where the Side I signature collapses.

`argmax` maximises per-file accuracy, but the score is **macro F1** over three
heavily imbalanced classes, so a missed Side I/II file is very costly. For folder
/batch inference (the submission CSV) `rail.predict_folder` therefore applies a
**fault budget**: rank files by `max(p_SideI, p_SideII)` and flag the top-k as
faults (`k = round(0.176 · n_files)` = 12 of 68, chosen by out-of-fold 68-file
validation), each taking its higher-probability side. This raised the held-out
macro F1 from 0.712 (argmax) to **0.805** by recovering the two faults that sat
just below the argmax boundary; single-file predictions keep `argmax`.
Investigated and rejected on the evidence: speed augmentation, mirror/TTA,
hierarchical models, curve/coherence proxies, order-domain phase features, and
1D/2D CNNs.
...
The app additionally tracks **cumulative** damage per **component**. A free-text
component field sits at the top of the SHM page and is set before uploading, so
every segment is filed under the component it came from. Miner's rule is
additive, so the state is a tiny Markov summary (`D`, `n_segments`, `n_samples`,
`rate`) persisted by `state_store.py` (Cloud Storage, or local JSON); the SHM
view shows the running D, segments to D = 1 and a table of all tracked
components, and `subsystems/shm/stream_watch.py` ingests a folder of segments
automatically with threshold alerts. The submission CSV stays **per segment**.
