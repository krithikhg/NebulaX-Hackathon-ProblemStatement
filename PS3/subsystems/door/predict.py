...
def predict(input_path, templates: dict, thresholds: dict, statistic: str) -> pd.DataFrame:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Door subsystem inference")
    ap.add_argument("--input", required=True, help="continuous door-stream CSV to score")
    ap.add_argument("--output", required=True, help="destination predictions CSV")
    ap.add_argument("--train", default=str(DEFAULT_TRAIN), help="labelled training stream CSV")
    ap.add_argument("--train-answer", default=str(DEFAULT_TRAIN_ANSWER),
                     help="Train_Segments_Answer.csv for --train")
    args = ap.parse_args()

    templates, thresholds, statistic = fit(args.train, args.train_answer)
    out = predict(args.input, templates, thresholds, statistic)

    out_path = pathlib.Path(args.output)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out.to_csv(out_path, index=False)
    print(f"wrote {len(out)} predicted segment(s) -> {out_path}")
